...
final class AudioRecorderManager: NSObject {
  ...
  private func prepareRecording() {
    ...
    do {
      ...
      // Set category and mode for recording
  ...
  func startRecordingDirectly() {
    ...
    DispatchQueue.global(qos: .userInitiated).async { [weak self] in
      ...
      do {
        let audioSession = AVAudioSession.sharedInstance()
        // Ensure category is set before activating
        try audioSession.setCategory(.record, mode: .default, options: [])
        try audioSession.setActive(true)
        
        DispatchQueue.main.async { [weak self] in
          guard let self = self, !self.isCancelled else {
            // If cancelled, deactivate the session we just activated
  ...
  func stopRecording() {
    // Mark as cancelled to prevent race condition with startRecordingDirectly
    isCancelled = true
    audioRecorder?.stop()
    
    // Move audio session deactivation off main thread
    DispatchQueue.global(qos: .userInitiated).async { [weak self] in
